...
from tasks.kylin.assets.assets_kylin import *
from tasks.kylin.interact import Interact
    ...
    def _kylin_affair(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if self.handle_reward():
                continue
            if self.appear_then_click(CAT_ALTAR_UNLOCK):
                logger.info("Get cat altar reward")
                timeout.reset()
                continue
            if self.appear_then_click(GET_SHRIMP_BALL_UNLOCK):
